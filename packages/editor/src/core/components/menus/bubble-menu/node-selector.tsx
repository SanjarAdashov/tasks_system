...
export function BubbleMenuNodeSelector(props: Props) {
  ...
  return (
    ...
    >
      <section className="gts-glass-popover-surface mt-1 flex max-h-[90vh] w-48 flex-col overflow-y-scroll rounded-md border-[0.5px] border-strong bg-surface-1 px-2 py-2.5 shadow-raised-200">
        {items.map((item) => (
          <button
            key={item.name}
            type="button"
