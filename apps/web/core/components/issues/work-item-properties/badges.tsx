...
const formatValue = (
  ...
  if (property.property_type === "SINGLE_SELECT" || property.property_type === "MULTI_SELECT") {
    ...
    const optionNames = selectedIds.map(
      (optionId) => property.options.find((option) => option.id === optionId)?.name ?? String(optionId)
    );
...
export function IssuePropertyBadges(props: Props) {
  ...
  const badges = useMemo(
    () =>
      (properties ?? [])
        .map((property) => {
          const formattedValue = formatValue(property, issue.property_values?.[property.id]);
    ...
    [issue.property_values, properties]
  );
